...
mysql 설치를 진행한다.
https://dev.mysql.com/downloads/mysql (LTS버전을 선택한다.)

mysql 설치 중 오류가 발생하면 vs code 라이브러리를 설치
...
```
##데이터 베이스
- 24시시간 365일동안 자료를 보관하며 서비스를 제공함
- 
...
SQL : Structured(구조화된) Query(질의) Language(언어) 표준화된 문법
  ...
  조작어(DML : Data Manipulation Language))
		...
		U(자료수정) : UPDATA 테이블명 SET(필드명 = 값) WHERE 조건
		D(자료삭제) : DELETE FROM 테이블 WHERE 조
